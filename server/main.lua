...
RegisterNetEvent('voice:PlayerConnected', function()
  local source = source;

  -- if the player already exists in the pool, then kick him
  if GetTableIndexBySource(source) then
    return DropPlayer(source,
      '[voice] | Player triggered "voice:PlayerConnected", although the player already existed');
  end

  -- create player table
  table_insert(voice.PlayerPool, {
    source = source,
  ...
  TriggerClientEvent('voice:PlayerLoaded', source, Config);
  -- update the player pool for all existing clients
  TriggerClientEvent('voice:PlayerPoolChanged', -1, voice.PlayerPool);
end)
...
local function toggleVoiceRange(player)
  ...
  for _, range in pairs(Config.Ranges) do
    ...
    if voice.PlayerPool[index].range == range then
      ...
      TriggerClientEvent('voice:VoiceRangeChanged', player, newRange, Index);
...
RegisterNetEvent('voice:cmd:toggleVoiceRange', function()
...
RegisterNetEvent('voice:cmd:pressedRadio', function()
  local s = source;
  local index = GetTableIndexBySource(s);
  -- set the player as talking on the radio
  voice.PlayerPool[index].radioActive = true;
  -- send all clients the new player pool
  TriggerClientEvent('voice:PlayerPoolChanged', -1, voice.PlayerPool);
end);

-- and this on release
RegisterNetEvent('voice:cmd:releasedRadio', function()
  local s = source;
  local index = GetTableIndexBySource(s);
  -- set the player as non talking on the radio
  voice.PlayerPool[index].radioActive = false;
  -- send all clients the new player pool
  TriggerClientEvent('voice:PlayerPoolChanged', -1, voice.PlayerPool);
end);
...
exports('MutePlayer', function(target, bool)
  ...
  voice.PlayerPool[index].muted = bool;
  -- send all clients the new player pool
  TriggerClientEvent('voice:PlayerPoolChanged', -1, voice.PlayerPool);
end)
...
local function addPlayerToPool(target, id, poolData)
  ...
  if not voice[poolData.name][value] then
    -- if the pool with the specified doesn't already exist, then initiate it
    voice[poolData.name][value] = {};
  end

  -- insert the original source to the pool
  table_insert(voice[poolData.name][value], target);

  -- update the player data
  voice.PlayerPool[index][poolData.value] = value;

  -- update the call/radio pool for all clients
  -- these events are simply for implementation in other scripts
  TriggerClientEvent('voice:' .. poolData.name .. 'Changed', -1, voice[poolData.name]);
  TriggerClientEvent('voice:RadioChannelChanged', target, value);
...
local function removePlayerFromPool(target, id, poolData)
  ...
  -- update the player data
  voice.PlayerPool[index][poolData.value] = nil;

  -- update the call/radio pool for all clients
  -- these events are simply for implementation in other scripts
  TriggerClientEvent('voice:' .. poolData.name .. 'Changed', -1, voice[poolData.name]);
  TriggerClientEvent('voice:RadioChannelChanged', target, nil);
  -- update the player pool for all clients
  TriggerClientEvent('voice:PlayerPoolChanged', -1, voice.PlayerPool);
end
...
AddEventHandler('playerDropped', function()
  ...
  for _, PoolData in pairs(voice.PlayerPool) do
    ...
    if PoolData.source == source then
      ...
      table_remove(voice.PlayerPool, _);
      break;
    end
  end
